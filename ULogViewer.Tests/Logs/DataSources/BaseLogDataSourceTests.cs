...
abstract class BaseLogDataSourceTests : ApplicationBasedTests
{
	...
	public void DisposingSourceWhenUsingTest()
	{
		this.TestOnApplicationThread(async () =>
		{
			...
			using (var _ = provider.CreateSource(options))
			{ }

			// dispose when ready to open reader
			using (var source = provider.CreateSource(options))
			{
				await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.ReadyToOpenReader, new CancellationTokenSource(5000).Token);
			}
			...
			using (var source = provider.CreateSource(options))
			{
				...
				await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.OpeningReader, new CancellationTokenSource(5000).Token);
				source.Dispose();
				try
				{
			...
			using (var source = provider.CreateSource(options))
			{
				...
				using (var reader = await source.OpenReaderAsync())
					Assert.IsNotNull(reader.ReadLine());
				await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.Preparing, new CancellationTokenSource(5000).Token);
			}
	...
	public void ReaderOpeningCancellationTest()
	{
		this.TestOnApplicationThread(async () =>
		{
			...
			for (var i = 0; i < 10; ++i)
			{
				...
				await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.ReaderOpened, new CancellationTokenSource(5000).Token);
				cancellationTokenSource.Cancel();
	...
	async Task ReadingFromSourceTest(ILogDataSourceProvider provider)
	{
		...
		if (source.State == LogDataSourceState.ReaderOpened)
			await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.ClosingReader, new CancellationTokenSource(5000).Token);
		...
		// dispose source
		source.Dispose();
		await source.WaitForPropertyChangeAsync(nameof(ILogDataSource.State), it => it.State == LogDataSourceState.Disposed, new CancellationTokenSource(5000).Token);
	}
}