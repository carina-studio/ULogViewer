...
partial class DisplayableLogGroup : BaseDisposable, IApplicationObject, ILogGroup
{
	...
	public DisplayableLogGroup(LogProfile profile)
	{
		...
		this.updateTextHighlightingDefSetAction = new(() =>
		{
			...
			definitions.Clear();
			if (this.activeInclusiveTextFilters.IsNotEmpty())
			{
	...
	/// <summary>
	/// Get or set list of text filters (inclusionary) which are applied on the group.
	/// </summary>
	public IList<Regex> ActiveInclusiveTextFilters
	{
		get => this.activeInclusiveTextFilters;
		set
		{
			this.VerifyAccess();
			this.VerifyDisposed();
