...
unsafe class DisplayableLog : IApplicationObject, IDisposable, INotifyPropertyChanged
{
	...
	public IULogViewerApplication Application =>
		...
		?? IAppSuiteApplication.CurrentOrNull as IULogViewerApplication
...
#pragma warning restore CS8603
	...
	IStringSource? FormatTimestamp(DateTime? timestamp)
	{
		...
		try
		{
			...
			return new SimpleStringSource(timestamp.Value.ToString(CultureInfo.InvariantCulture));
		}
		catch
		{
			return null;
	...
	public string LevelString
	{
		get
		{
			...
			if (propertyName != nameof(Level) && this.TryGetProperty(propertyName, out s) && s is not null)
				return s;
	...
	{
		var propertyChangedHandlers = this.PropertyChanged;
		if (propertyChangedHandlers is not null && this.Log.ProcessId.HasValue)
	...
	{
		var propertyChangedHandlers = this.PropertyChanged;
		if (propertyChangedHandlers is not null && this.Log.ThreadId.HasValue)
