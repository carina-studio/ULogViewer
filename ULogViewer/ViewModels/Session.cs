...
#pragma warning restore CS0612
		...
		void AddLogFile(LogDataSourceParams<string>? param)
		{
			...
			// create log reader
			if (!this.CreateLogReader(dataSource, param.Precondition))
		...
		// Create log reader.
		bool CreateLogReader(ILogDataSource dataSource, LogReadingPrecondition precondition)
		{
		...
		/// </summary>
		public bool IsProVersionActivated { get => this.GetValue(IsProVersionActivatedProperty); }
...
#pragma warning restore IDE0220
		...
		void RestoreLogReaders()
		{
			...
			foreach (var logReaderOption in this.savedLogReaderOptions)
			{
				...
				if (isFileNameRequired && !isFileNameSet && newDataSourceOptions.IsOptionSet(nameof(LogDataSourceOptions.FileName)))
				{
					for (var i = this.logFileInfoList.Count - 1; i >= 0; --i)
					{
						...
						if (logFileInfo.IsPredefined)
						{
							...
							this.logFileInfoList.Add(new LogFileInfoImpl(this, logFileInfo.FileName, logFileInfo.LogReadingPrecondition));
						}
						break;
					}
				}
				...
				if (dataSource != null)
				{
					if (!this.CreateLogReader(dataSource, logReaderOption.Precondition))
					{
		...
		void StartReadingLogs()
		{
			...
			if (canStartReading)
			{
				...
				if (dataSource != null)
				{
					if (!this.CreateLogReader(dataSource, new()))
					{
