			...
			try
			{
				...
				foreach (var log in logs)
				{
					...
					foreach (var propertyName in propertyGetters.Keys)
					{
						...
						{
							jsonWriter.WriteStringValue(hasTimestampFormat
								? dateTimeValue.ToString(timestampFormat, timestampCultureInfo) 
								: dateTimeValue.ToString(timestampCultureInfo));
						}
						else if (value is TimeSpan timeSpanValue)
						{
							jsonWriter.WriteStringValue(hasTimeSpanFormat 
								? timeSpanValue.ToString(timeSpanFormat, timeSpanCultureInfo)
								: timeSpanValue.ToString());
						}
						else if (value is int intValue)
							jsonWriter.WriteNumberValue(intValue);
						else if (value is LogLevel level)
						{
							jsonWriter.WriteStringValue(logLevelMap.TryGetValue(level, out var str) 
								? str 
								: level.ToString());
						}
						else
