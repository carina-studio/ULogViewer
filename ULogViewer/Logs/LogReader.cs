...
class LogReader : BaseDisposable, IApplicationObject, INotifyPropertyChanged
{
	...
	public LogReader(ILogGroup? group, ILogDataSource dataSource)
	{
		// check thread
...
#pragma warning restore IDE0220
	...
	async void StartReadingLogs()
	{
		...
		_ = this.LogGroup is not null
			? this.LogGroup.SelectLogsReadingTaskFactory(this.DataSource).StartNew(() => this.ReadLogs(readingToken, reader, cancellationToken), CancellationToken.None)
			: Task.Run(() => this.ReadLogs(readingToken, reader, cancellationToken), CancellationToken.None);
	}
