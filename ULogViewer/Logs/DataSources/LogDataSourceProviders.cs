...
static class LogDataSourceProviders
{
	...
	/// </summary>
	public static ILogDataSourceProvider Empty => empty ?? throw new InvalidOperationException($"{nameof(LogDataSourceProviders)} is not initialized yet.");
	...
	public static async Task InitializeAsync(IULogViewerApplication app)
	{
		// check state
		app.VerifyAccess();
		if (LogDataSourceProviders.app is not null)
		{
		...
		// create logger
		logger = app.LoggerFactory.CreateLogger(nameof(LogDataSourceProviders));
