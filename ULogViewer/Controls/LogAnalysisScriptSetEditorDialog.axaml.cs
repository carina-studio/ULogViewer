	...
	async Task OnOpenedAsync()
	{
		    ...
		    && !this.Application.ProductManager.IsProductActivated(Products.Professional))
		{
			if (!LogAnalysisScriptSetManager.Default.CanAddScriptSet)
			{
				...
				await Task.Delay(300, CancellationToken.None); // [Workaround] need to wait for initial size ready
				if (this.IsOpened)
				{
					await new MessageDialog
					{
...
#pragma warning restore CA1822
	...
	/// <summary>
	/// Open window to show logs generated by running scripts.
	/// </summary>
#pragma warning disable CA1822
	public void OpenScriptLogWindow() => ScriptManager.Default.OpenLogWindow();
#pragma warning restore CA1822
	...
	{
		// check state, a disabled editor is either being closed or not interactive yet
		if (!this.IsOpened || !this.IsEnabled || this.isRequestingEnablingRunningScript || this.Settings.GetValueOrDefault(AppSuite.SettingKeys.EnableRunningScript))
			return;
